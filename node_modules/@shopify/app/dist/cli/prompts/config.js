...
import { AppSchema, getAppScopes, getAppScopesArray } from '../models/app/app.js';
...
export async function confirmPushChanges(force, configuration, app, schema = AppSchema) {
    if (force)
        return true;
    const remoteConfiguration = mergeAppConfiguration(configuration, app, false);
...
export function buildDiffConfigContent(localConfig, remoteConfig, schema = AppSchema, renderNoChanges = true) {
    if (getAppScopes(localConfig) !== '') {
        setPathValue(localConfig, 'access_scopes.scopes', getAppScopesArray(localConfig).join(','));
    }
    const [updated, baseline] = deepDifference({ ...rewriteConfiguration(schema, localConfig), build: undefined }, { ...rewriteConfiguration(schema, remoteConfig), build: undefined });
