...
export function getAppVersionedSchema(specs) {
    const isConfigSpecification = (spec) => spec.appModuleFeatures().includes('app_config');
    const schema = specs
...
export class App {
    ...
    constructor(name, idEnvironmentVariableName, directory, packageManager, configuration, nodeDependencies, webs, extensions, usesWorkspaces, dotenv, errors, specifications, configSchema, remoteBetaFlags) {
        this.name = name;
    ...
    get allExtensions() {
        return this.realExtensions.filter((ext) => !ext.isAppConfigExtension || (this.useVersionedAppConfig && this.includeConfigOnDeploy));
    }
    ...
    webhooksConfiguration(configuration) {
        return {
...
export class EmptyApp extends App {
    constructor(specifications, betas) {
        const configuration = { scopes: '', extension_directories: [], path: '' };
        const configSchema = getAppVersionedSchema(specifications ?? []);
        super('', '', '', 'npm', configuration, {}, [], [], false, undefined, undefined, specifications, configSchema, betas);
    }
}
/**
